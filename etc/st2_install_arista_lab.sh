...
#
# Arista changes:
#  Arista uses Guacamole for RDP. Nginx is used as a reverse proxy for outside traffic
#  to the local Guacamole port. The config for this lives in /etc/nginx/sites-enabled/default
#  Nginx for Guacamole listens on 0.0.0.0:8181 and reverse proxies to 127.0.0.1:8080
#
#  This script, by default, upgrades and installs nginx from the upstream repo.
#  Unfortunately this breaks the Arista box because the nginx version installed on it is
#  nginx-core (from Ubuntu repo) instead of from the nginx repo.
#
#  To fix this problem i modified the script below to assume that nginx is already installed.
#  We simply generate an SSL key, put the StackStorm nginx config in place and bounce nginx.
#
#  FYI, during install this causes Guacamole (RDP) to lose connection for a second.
#  You'll see a red pop-up saying you've lost connection. Simply hit "RECONNECT" and you
#  should be brought back into your Guacamole session.
#
#
...
#   sudo apt-get install -y st2web${ST2WEB_PKG_VERSION} nginx
  ...
  # Install st2web and nginx
  sudo apt-get install -y st2web${ST2WEB_PKG_VERSION}
